...
class GenotypeResult:
    def __init__(
        self,
        copy_numbers,
        recruited_reads_count,
        spanning_reads_count,
        flanking_reads_count,
        max_likelihood,
    ):
...
class SelectedRead:
    def __init__(
        self, sequence, logp, vpath, mapq=None, reference_start=None, query_name=None
    ):
        self.sequence = sequence
        self.logp = logp
        self.vpath = vpath
...
class VNTRFinder:
    ...
    def build_vntr_matcher_hmm(self, copies, flanking_region_size=100):
        ...
        vntr_matcher = get_read_matcher_model(
            left_flanking_region, right_flanking_region, patterns, copies
        )
        return vntr_matcher
    ...
    def get_keywords_for_filtering(self, short_reads=True, keyword_size=21):
        ...
        locus = (
            self.reference_vntr.left_flanking_region[-15:]
            + vntr
            + self.reference_vntr.right_flanking_region[:15]
        )
        queries = []
        ...
        if not short_reads:
            queries = [
                self.reference_vntr.left_flanking_region[-80:],
                self.reference_vntr.right_flanking_region[:80],
            ]
        queries = set(queries)
        return queries
    ...
    def is_true_read(self, read):
        ...
        if (
            reference_name == self.reference_vntr.chromosome
            and self.vntr_start - len(read.seq) < read_start < self.vntr_end
        ):
            return True
        return False
    ...
    def get_min_score_to_select_a_read(self, read_length):
        if (
            self.reference_vntr.scaled_score is None
            or self.reference_vntr.scaled_score == 0
        ):
            return None
    ...
    def recruit_read(self, logp, vpath, min_score_to_count_read, read_sequence):
        ...
        if (
            get_flanking_regions_matching_rate(
                vpath, read_sequence, left_flank, right_flank
            )
            < 0.90
        ):
            return False
        ...
        if (
            min_score_to_count_read is None
            and matches >= 0.9 * read_length
            and logp > -read_length
        ):
            return True
        return False

    def process_unmapped_read_with_dnn(
        self,
        read_segment,
        hmm,
        recruitment_score,
    ...
    def identify_frameshift(
        self,
        location_coverage,
        observed_indel_transitions,
        expected_indels,
        error_rate=0.01,
    ):
        ...
        sequencing_error_prob = binom.pmf(
            observed_indel_transitions, location_coverage, error_rate
        )
        frameshift_prob = binom.pmf(
            observed_indel_transitions, location_coverage, expected_indels
        )
    ...
    def find_frameshift_from_selected_reads(self, selected_reads):
        ...
        for read in selected_reads:
            ...
            current_repeat = None
            repeating_bps_in_data += get_number_of_repeat_bp_matches_in_vpath(
                read.vpath
            )
            for i in range(len(visited_states)):
                if visited_states[i].endswith("fix") or visited_states[i].startswith(
                    "M"
                ):
                    continue
                ...
                if current_repeat is None or current_repeat >= len(repeats_lengths):
                    continue
                if not visited_states[i].startswith("I") and not visited_states[
                    i
                ].startswith("D"):
                    continue
                ...
                if state.startswith("I"):
                    state += get_emitted_basepair_from_visited_states(
                        visited_states[i], visited_states, read.sequence
                    )
                if (
                    abs(
                        repeats_lengths[current_repeat]
        ...
        frameshift_candidate = (
            sorted_mutations[-1] if len(sorted_mutations) else (None, 0)
        )
        logging.info(sorted(repeats_lengths_distribution))
        logging.info(
            "Frameshift Candidate and Occurrence %s: %s" % frameshift_candidate
        )
        logging.info(
            "Observed repeating base pairs in data: %s" % repeating_bps_in_data
        )
        avg_bp_coverage = (
            float(repeating_bps_in_data) / self.reference_vntr.get_length() / 2
        )
        logging.info("Average coverage for each base pair: %s" % avg_bp_coverage)

        expected_indel_transitions = 1 / avg_bp_coverage
        if self.identify_frameshift(
            avg_bp_coverage, frameshift_candidate[1], expected_indel_transitions
        ):
    ...
    def read_flanks_repeats_with_confidence(self, vpath, read_sequence):
        ...
        left_flank = self.reference_vntr.left_flanking_region
        logging.debug(
            "get_flanking_regions_matching_rate: accuracy filter T {} F {}".format(
                get_flanking_regions_matching_rate(
                    vpath, read_sequence, left_flank, right_flank, accuracy_filter=True
                ),
                get_flanking_regions_matching_rate(
                    vpath, read_sequence, left_flank, right_flank, accuracy_filter=False
                ),
            )
        )
        if (
            ...
            return False
        if (
            get_left_flanking_region_size_in_vpath(vpath)
            > self.minimum_left_flanking_size
        ):
            if (
                get_right_flanking_region_size_in_vpath(vpath)
                > self.minimum_right_flanking_size
            ):
                return True
        return False

    def check_if_flanking_regions_align_to_str(
        self, read_str, read_id, length_distribution, spanning_reads
    ):
        ...
        if right_align[3] < left_align[3]:
            return
        spanning_reads.append(
            LoggedRead(
                sequence=read_str[
                    left_align[3] : right_align[3] + flanking_region_size
                ],
                read_id=read_id,
        ...
        length_distribution.append(
            right_align[3] - (left_align[3] + flanking_region_size)
        )

    def check_if_pacbio_read_spans_vntr(
        self, sema, read, length_distribution, spanning_reads
    ):
        self.check_if_flanking_regions_align_to_str(
            str(read.seq).upper(), read.query_name, length_distribution, spanning_reads
        )
    ...
    def check_if_pacbio_mapped_read_spans_vntr(
        self, sema, read, length_distribution, spanning_reads
    ):
        ...
        if (
            first_aligned_position <= vntr_start - min_flanking_bp
            and vntr_end + min_flanking_bp < last_aligned_position
        ):
            ...
            left_flanking_bp = 0
            right_flanking_bp = 0
            for read_pos, ref_pos in enumerate(
                ...
                if (
                    region_start <= ref_pos < vntr_end + hmm_flanking_region_size
                ):  # HMM-modeled VNTR region
            ...
            if (
                left_flanking_bp < min_flanking_bp
                or right_flanking_bp < min_flanking_bp
            ):
                logging.debug(
                    "Rejecting the read {} due to short spanning regions".format(
                        read.query_name
                    )
                )
                sema.release()
            ...
            if (
                ...
                length_distribution.append(
                    len(result_seq) - left_flanking_bp - right_flanking_bp
                )
        sema.release()
    ...
    def get_spanning_reads_of_unaligned_pacbio_reads(self, unmapped_filtered_reads):
        ...
        for read in unmapped_filtered_reads:
            sema.acquire()
            p = Process(
                target=self.check_if_pacbio_read_spans_vntr,
                args=(sema, read, shared_length_distribution, shared_spanning_reads),
            )
        ...
        for p in process_list:
            p.join()
        logging.info(
            "length_distribution of unmapped spanning reads: %s"
            % list(shared_length_distribution)
        )
    ...
    def get_spanning_reads_of_aligned_pacbio_reads(self, alignment_file):
        ...
        samfile = pysam.AlignmentFile(
            alignment_file, read_mode, reference_filename=self.reference_filename
        )
        reference = get_reference_genome_of_alignment_file(samfile)
        chromosome = (
            self.reference_vntr.chromosome
            if reference == "HG19"
            else self.reference_vntr.chromosome[3:]
        )
        process_list = []
        for read in samfile.fetch(chromosome, region_start, region_end):
            ...
            sema.acquire()
            p = Process(
                target=self.check_if_pacbio_mapped_read_spans_vntr,
                args=(sema, read, length_distribution, mapped_spanning_reads),
            )
            process_list.append(p)
            p.start()
        ...
        logging.info(
            "length_distribution of mapped spanning reads: %s"
            ...
            for i in range(len(ru_counts)):
                ...
                for j in range(len(ru_counts)):
                    ...
                    if (ci, cj) not in prs.keys():
                        prs[(ci, cj)] = []
                    prs[(ci, cj)].append(
                        self.get_conditional_likelihood(ck, ci, cj, ru_counts, r, r_e)
                        ** occ
                    )
    ...
    def get_dominant_copy_numbers_from_spanning_reads(
        self, spanning_reads, log_pacbio_reads, accuracy_filter
    ):
        ...
        if accuracy_filter:
            ...
            self.minimum_left_flanking_size = (
                settings.ACCURACY_FILTER_MIN_LEFT_FLANKING_SIZE
            )
            self.minimum_right_flanking_size = (
                settings.ACCURACY_FILTER_MIN_RIGHT_FLANKING_SIZE
            )
        ...
        for spanning_read in spanning_reads:
            ...
            if log_pacbio_reads:
                ...
                if self.read_flanks_repeats_with_confidence(vpath, read_sequence):
                    logging.debug(
                        "spanning read %s sourced from %s visited states :%s"
                        % (
                            spanning_read.read_id,
                            spanning_read.source.name,
                            visited_states,
                        )
                    )
                else:
                    logging.debug(
                        "flanking read %s sourced from %s visited states :%s"
                        % (
                            spanning_read.read_id,
                            spanning_read.source.name,
                            visited_states,
        ...
        copy_numbers, max_prob = self.find_genotype_based_on_observed_repeats(
            observed_copy_numbers
        )

        genotype_string = (
            "/".join([str(cn) for cn in sorted(copy_numbers)])
    ...
    def get_haplotype_copy_numbers_from_spanning_reads(self, spanning_reads):
        ...
        for haplotype in haplotypes:
            ...
            rev_logp, rev_vpath = vntr_matcher.viterbi(
                str(Seq(haplotype).reverse_complement())
            )
            if logp < rev_logp:
                vpath = rev_vpath
    ...
    def find_ru_counts_with_naive_approach(self, length_dist, spanning_reads):
        ...
        if len(flanking_region_lengths) > 0:
            return tuple(
                [round(flanking_region_lengths[0] / len(self.reference_vntr.pattern))]
                * 2
            )
        else:
            return None
    ...
    def find_ru_counts_from_average_flanking_region_distance(self, length_dist):
        if len(length_dist):
            ru_counts_list = [
                round(length / len(self.reference_vntr.pattern))
                for length in length_dist
            ]
            ru_count_frequencies = Counter(ru_counts_list)
            copy_numbers = [ru_count_frequencies[0][0]]
            if (
                len(ru_count_frequencies.keys()) > 1
                and ru_count_frequencies[1][1] > ru_count_frequencies[0][1] / 5
            ):
    ...
    @time_usage
    def find_repeat_count_from_pacbio_alignment_file(
        self, alignment_file, unmapped_filtered_reads, log_pacbio_reads, accuracy_filter
    ):
        logging.debug(
            "finding repeat count from pacbio alignment file for %s"
            % self.reference_vntr.id
        )

        unaligned_spanning_reads, length_dist = (
            self.get_spanning_reads_of_unaligned_pacbio_reads(unmapped_filtered_reads)
        )
        ...
        copy_numbers, max_prob = self.get_dominant_copy_numbers_from_spanning_reads(
            spanning_reads, log_pacbio_reads, accuracy_filter
        )

        return GenotypeResult(
            copy_numbers, len(spanning_reads), len(spanning_reads), 0, max_prob
        )

    @time_usage
    def find_repeat_count_from_pacbio_reads(
        self, unmapped_filtered_reads, log_pacbio_reads, accuracy_filter, naive=False
    ):
        logging.debug(
            "finding repeat count from pacbio reads file for %s"
            % self.reference_vntr.id
        )
        logging.debug("accuracy filter is {}".format(accuracy_filter))
        spanning_reads, length_dist = self.get_spanning_reads_of_unaligned_pacbio_reads(
            unmapped_filtered_reads
        )
        max_prob = 0
        if naive:
            copy_numbers = self.find_ru_counts_with_naive_approach(
                length_dist, spanning_reads
            )  # No max_prob value
        else:
            copy_numbers, max_prob = self.get_dominant_copy_numbers_from_spanning_reads(
                spanning_reads, log_pacbio_reads, accuracy_filter
    ...
    @time_usage
    def iteratively_update_model(
        self, alignment_file, unmapped_filtered_reads, selected_reads, hmm
    ):
        ...
        for i in range(max_steps):
            old_fitness = fitness
            current_vpaths = [
                (read.sequence, read.vpath)
                for read in updated_selected_reads + reference_repeats
            ]
            hmm = get_read_matcher_model(
            ...
            updated_selected_reads = self.select_illumina_reads(
                alignment_file, unmapped_filtered_reads, False, hmm
            )
            fitness = sum([read.logp for read in selected_reads])
    ...
    def select_illumina_reads(
        self, alignment_file, unmapped_filtered_reads, update=False, hmm=None
    ):
        ...
        for read in samfile.fetch(chromosome, vntr_start, vntr_end):
            ...
            read_end = (
                ...
                else read.reference_start + len(read.seq)
            )
            if (
                vntr_start - read_length < read.reference_start < vntr_end
        ...
        for read_segment in unmapped_filtered_reads:
            ...
            if dnn_model is None:
                ...
                self.process_unmapped_read_with_dnn(
                    str(read_segment.seq),
                    hmm,
                    recruitment_score,
                    vntr_bp_in_unmapped_reads,
        ...
        logging.debug(
            "vntr base pairs in unmapped reads: %s" % vntr_bp_in_unmapped_reads.value
        )
    ...
    def find_frameshift_from_alignment_file(
        self, alignment_file, unmapped_filtered_reads
    ):
        logging.debug(
            "finding frameshift from alignment file for %s" % self.reference_vntr.id
        )

        selected_reads = self.select_illumina_reads(
            alignment_file, unmapped_filtered_reads
        )
        return self.find_frameshift_from_selected_reads(selected_reads)

    @time_usage
    def get_ru_count_with_coverage_method(
        self, pattern_occurrences, total_counted_vntr_bp, average_coverage
    ):
    ...
    @time_usage
    def find_repeat_count_from_alignment_file(
        self,
        alignment_file,
        unmapped_filtered_reads,
        accuracy_filter,
        ...
        logging.debug(
            "finding repeat count from alignment file for %s" % self.reference_vntr.id
        )
        logging.debug("accuracy filter is {}".format(accuracy_filter))
        logging.debug(
            "left and right min flanking size: {} {}".format(
                self.minimum_left_flanking_size, self.minimum_right_flanking_size
            )
        )

        selected_reads = self.select_illumina_reads(
            alignment_file, unmapped_filtered_reads, update
        )
        ...
        for selected_read in selected_reads:
            ...
            total_counted_vntr_bp += get_number_of_repeat_bp_matches_in_vpath(
                selected_read.vpath
            )
            left_flanking_size_vpath = get_left_flanking_region_size_in_vpath(
                selected_read.vpath
            )
            right_flanking_size_vpath = get_right_flanking_region_size_in_vpath(
                selected_read.vpath
            )
            ...
            logged_read = LoggedRead(
                sequence=selected_read.sequence,
                read_id=selected_read.query_name,
                source=read_source,
            )
            if self.read_flanks_repeats_with_confidence(
                selected_read.vpath, selected_read.sequence
            ):
                logging.debug(
                    "spanning read %s sourced from %s visited states :%s"
                ...
                logging.debug(
                    "Rejecting read {} for low match rate in flanking regions".format(
        ...
        max_flanking_repeat = [
            r
            for r in flanking_repeats
            if r == max(flanking_repeats) and r >= min_valid_flanked
        ]
        ...
        if accuracy_filter:
            ...
            if sorted(modified_repeats) != sorted(covered_repeats):
                logging.debug(
                    "Modifying repeat counts, observed: {} modified: {}".format(
        ...
        exact_genotype, max_prob = self.find_genotype_based_on_observed_repeats(
            covered_repeats + max_flanking_repeat
        )
        ...
        if (
            average_coverage is None
            or average_coverage is False
            or average_coverage == 0
        ):
            ...
            return GenotypeResult(
                ...
                len(selected_reads),
                len(covered_repeats),
                len(flanking_repeats),
                max_prob,
            )

        logging.info("Average coverage is {}".format(average_coverage))
        pattern_occurrences = sum(flanking_repeats) + sum(covered_repeats)
        estimated_genotype = self.get_ru_count_with_coverage_method(
            pattern_occurrences, total_counted_vntr_bp, average_coverage
        )
        return GenotypeResult(
            estimated_genotype,
            len(selected_reads),
            ...
            len(flanking_repeats),
            0,
        )  # No probability for the estimated genotype

    def find_repeat_count_from_short_reads(
        ...
        return self.find_repeat_count_from_alignment_file(
    ...
    def train_classifier_threshold(self, reference_file, read_length=150):
        ...
        simulated_false_filtered_reads = self.simulate_false_filtered_reads(
            reference_file
        )

        processed_true_reads = self.find_hmm_score_of_simulated_reads(
            hmm, simulated_true_reads
        )
        processed_false_reads = self.find_hmm_score_of_simulated_reads(
            hmm, simulated_false_filtered_reads
        )

        recruitment_score = self.find_recruitment_score_threshold(
    ...
    def simulate_false_filtered_reads(self, reference_file, min_match=3):
        ...
        def get_hash(string):
            ...
            for k in range(len(string)):
                result = (
                    result + alphabet[string[k].upper()] * (4 ** (keyword_size - k - 1))
                ) % m
            return result
        ...
        for fasta in fasta_sequences:
            ...
            for i in range(0, len(sequence) - keyword_size):
                if (
                    sequence[i].upper() not in "ACTG"
                    or sequence[i - 1 + keyword_size].upper() not in "ACTG"
                ):
                    continue
                if window_hash is None or sequence[i - 1].upper() not in "ACTG":
                    ...
                    continue
                window_hash -= alphabet[sequence[i - 1].upper()] * (
                    4 ** (keyword_size - 1)
                )
                window_hash = (
                    window_hash * 4 + alphabet[sequence[i - 1 + keyword_size].upper()]
                ) % m
                if window_hash in hashed_keywords:
                    if (
                        name == self.reference_vntr.chromosome
                        and vntr_start - read_size < i < vntr_end
                    ):
                        continue
                    if sequence[i : i + keyword_size].upper() in keywords:
                        match_positions.append(i)
                        if (
                            len(match_positions) >= min_match
                            and match_positions[-1] - match_positions[-min_match]
                            < read_size
                        ):
                            for j in range(
                                match_positions[-1] - read_size,
                                match_positions[-min_match],
                                5,
                            ):
                                if "N" not in sequence[j : j + read_size].upper():
                                    false_filtered_reads.append(
                                        sequence[j : j + read_size]
                                    )
    ...
    def simulate_true_reads(self, read_length):
        ...
        for copies in range(1, len(self.reference_vntr.get_repeat_segments()) - 1):
            ...
            for i in range(1, 11):
                sim_reads.append(
                    (left_flank[-i:] + vntr_section + right_flank)[:read_length]
                )
                sim_reads.append(
                    (left_flank + vntr_section + right_flank[:i])[-read_length:]
                )
    ...
    @time_usage
    def find_recruitment_score_threshold(
        self, processed_true_reads, processed_false_reads
    ):
