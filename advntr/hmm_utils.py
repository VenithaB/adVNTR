...
def get_multiple_alignment_of_viterbi_paths(repeats_sequences, repeats_visited_states):
    ...
    for i, repeat_sequence in enumerate(repeats_sequences):
        ...
        individual_alignment = [
            state.split("_")[0] for state in repeats_visited_states[i]
        ]
...
def get_multiple_alignment_of_repeats_from_reads(sequence_vpath_list):
    ...
    for sequence, vpath in sequence_vpath_list:
        ...
        repeats, repeats_vstates = extract_repeating_segments_from_read(
            sequence, visited_states
        )
        repeats_sequences += repeats
        repeats_visited_states += repeats_vstates

    return get_multiple_alignment_of_viterbi_paths(
        repeats_sequences, repeats_visited_states
    )
...
def get_number_of_repeats_in_vpath(vpath):
    ...
    for i in range(len(visited_states)):
        if is_emitting_state(visited_states[i]):
            current_bp += 1
        if (
            visited_states[i].startswith("unit_start")
            and read_length - current_bp >= minimum_required_bp_in_repeat
        ):
            ...
            starts += 1
        if (
            visited_states[i].startswith("unit_end")
            and current_bp >= minimum_required_bp_in_repeat
        ):
...
def get_number_of_repeat_bp_matches_in_vpath(vpath):
    ...
    result = 0
    for i in range(len(visited_states)):
        if is_emitting_state(visited_states[i]) and not visited_states[i].endswith(
            "fix"
        ):
            result += 1
    return result
...
def get_flanking_regions_matching_rate(
    vpath, sequence, left_flank, right_flank, accuracy_filter=False, verbose=False
):
    visited_states = [state.name for idx, state in vpath[1:-1]]
    ...
    for i in range(len(visited_states)):
        ...
        if visited_states[i].endswith("prefix"):
            ...
            if (
                is_match_state(visited_states[i])
                and sequence[seq_index] == right_flank[hmm_state - 1]
            ):
        ...
        if visited_states[i].endswith("suffix"):
            ...
            if (
                is_match_state(visited_states[i])
                and sequence[seq_index] == left_flank[-(max_hmm_index - hmm_state + 1)]
            ):
    ...
    if accuracy_filter:
        ...
        right_rate = (
            ...
            if right_flanking_basepairs != 0
            else epsilon
        )
        left_rate = (
            float(left_flanking_matches) / left_flanking_basepairs
            if left_flanking_basepairs != 0
            else epsilon
        )
    else:
        right_rate = (
...
def get_left_flanking_region_size_in_vpath(vpath):
    ...
    result = 0
    for i in range(len(visited_states)):
        if is_emitting_state(visited_states[i]) and visited_states[i].endswith(
            "suffix"
        ):
            result += 1
    return result
...
def get_prefix_matcher_hmm(pattern):
    ...
    for i in range(len(pattern)):
        ...
        match_states.append(
            State(
                DiscreteDistribution(distribution_map),
    ...
    model.add_states(
        insert_states + match_states + delete_states + [unit_start, unit_end]
    )
    ...
    model.add_transition(
        insert_states[0], match_states[0], 1 - insert_error - delete_error
    )
    ...
    for i in range(0, len(pattern)):
        ...
        if i < len(pattern) - 1:
            model.add_transition(
                insert_states[i + 1],
                match_states[i + 1],
                1 - insert_error - delete_error,
            )
            model.add_transition(
                insert_states[i + 1], delete_states[i + 1], delete_error
            )

            model.add_transition(
                match_states[i],
                match_states[i + 1],
                1 - insert_error - delete_error - 0.01,
            )
            ...
            model.add_transition(
                delete_states[i], match_states[i + 1], 1 - insert_error - delete_error
            )
...
def get_suffix_matcher_hmm(pattern):
    ...
    for i in range(len(pattern)):
        model.add_transition(
            unit_start,
            match_states[i],
            (1 - insert_error - delete_error) / len(pattern),
        )
    ...
    for i in range(0, len(pattern)):
        ...
        if i < len(pattern) - 1:
            ...
            model.add_transition(
                match_states[i], match_states[i + 1], 1 - insert_error - delete_error
            )
            model.add_transition(match_states[i], delete_states[i + 1], delete_error)
...
def get_constant_number_of_repeats_matcher_hmm(patterns, copies, vpaths):
    ...
    if vpaths:
        ...
        transitions, emissions = build_profile_hmm_pseudocounts_for_alignment(
            settings.MAX_ERROR_RATE, alignment
        )
    else:
    ...
    for repeat in range(copies):
        ...
        model.add_transition(
            unit_start, match_states[0], transitions["unit_start"]["M1"]
        )
        model.add_transition(
            unit_start, delete_states[0], transitions["unit_start"]["D1"]
        )
        model.add_transition(
            unit_start, insert_states[0], transitions["unit_start"]["I0"]
        )

        model.add_transition(
            insert_states[0], insert_states[0], transitions["I0"]["I0"]
        )
        model.add_transition(
            insert_states[0], delete_states[0], transitions["I0"]["D1"]
        )
        model.add_transition(insert_states[0], match_states[0], transitions["I0"]["M1"])

        model.add_transition(
            delete_states[n], unit_end, transitions["D%s" % (n + 1)]["unit_end"]
        )
        model.add_transition(
            delete_states[n],
            insert_states[n + 1],
        ...
        model.add_transition(
            match_states[n], unit_end, transitions["M%s" % (n + 1)]["unit_end"]
        )
        model.add_transition(
            match_states[n],
            insert_states[n + 1],
        ...
        model.add_transition(
            insert_states[n + 1], unit_end, transitions["I%s" % (n + 1)]["unit_end"]
        )

        for i in range(1, len(matches) + 1):
            model.add_transition(
                match_states[i - 1], insert_states[i], transitions["M%s" % i]["I%s" % i]
            )
            model.add_transition(
                delete_states[i - 1],
                insert_states[i],
            ...
            model.add_transition(
                insert_states[i], insert_states[i], transitions["I%s" % i]["I%s" % i]
            )
            if i < len(matches):
                model.add_transition(
                    insert_states[i],
...
def get_variable_number_of_repeats_matcher_hmm(patterns, copies=1, vpaths=None):
    ...
    new_model = Model.from_matrix(
        mat, distributions, starts, ends, name=name, state_names=state_names, merge=None
    )
...
@time_usage
def get_read_matcher_model(
    left_flanking_region, right_flanking_region, patterns, copies=1, vpaths=None
):
