...
def load_unprocessed_vntrseek_data(vntrseek_output, chromosome=None, only_genic=False):
    ...
    with open(vntrseek_output) as input_file:
        ...
        for vntr_id, line in enumerate(input_lines):
            ...
            if only_genic and not is_vntr_close_to_gene(
                genes_info, chromosome_number, start, estimated_end
            ):
                continue
            vntrs.append(
                ReferenceVNTR(
...
def find_non_overlapping_vntrs(
    manager, vntrs, result, index, chrom=None, output_file=None, sema=None
):
    skipped_vntrs = set([])
    for i in range(len(vntrs)):
        if chrom is not None and vntrs[i].chromosome != chrom:
            continue
        estimated_end = (
            len(vntrs[i].pattern) * vntrs[i].estimated_repeats + vntrs[i].start_point
        )
        print(i, estimated_end - vntrs[i].start_point)
        if (
            i < len(vntrs) - 1
            and vntrs[i].chromosome == vntrs[i + 1].chromosome
            and estimated_end > vntrs[i + 1].start_point
        ):
        ...
        if i in skipped_vntrs:
            ...
            j = i + 1
            end_point = (
                len(vntrs[i].pattern) * len(repeat_segments) + vntrs[i].start_point
            )
            while (
                j < len(vntrs)
                and vntrs[i].chromosome == vntrs[j].chromosome
                and end_point > vntrs[j].start_point
            ):
                skipped_vntrs.add(j)
                j += 1
    with lock:
...
def process_vntrseek_data(
    unprocessed_vntrs_file, output_file="vntr_data/VNTRs.txt", chrom=None
):
    process_list = []
    ...
    for i in range(settings.CORES):
        ...
        p = Process(
            target=find_non_overlapping_vntrs,
            args=(manager, partial_input, partial_vntrs, i, chrom, output_file, sema),
        )
...
def identify_homologous_vntrs(vntrs, chromosome=None):
    for i in range(len(vntrs)):
        for j in range(i + 1, len(vntrs)):
            if chromosome and (
                chromosome != vntrs[i].chromosome and chromosome != vntrs[j].chromosome
            ):
                continue
...
def update_trained_score_in_database(vntr_id, scaled_recruitment_score):
    ...
    cursor = db.cursor()
    cursor.execute(
        """UPDATE vntrs SET scaled_score=? WHERE id=?""",
        (scaled_recruitment_score, vntr_id),
    )
    db.commit()
    db.close()


def update_gene_name_and_annotation_in_database(
    vntr_id, gene_name, annotation, db_file=None
):
    if db_file is None:
        db_file = settings.TRAINED_MODELS_DB
    db = sqlite3.connect(db_file)
    cursor = db.cursor()
    cursor.execute(
        """UPDATE vntrs SET gene_name=?, annotation=? WHERE id=?""",
        (gene_name, annotation, vntr_id),
    )
    db.commit()
    db.close()
...
def save_reference_vntr_to_database(ref_vntr, db_file=None):
    ...
    cursor.execute(
        ...
        (
            ...
            ref_vntr.annotation,
            ref_vntr.pattern,
            ref_vntr.left_flanking_region,
            ref_vntr.right_flanking_region,
            segments,
            ref_vntr.scaled_score,
        ),
    )
    db.commit()
    db.close()
...
def find_similar_region_for_vntr(sema, reference_vntr, ref_file, result_list):
    ...
    q = (
        reference_vntr.left_flanking_region[-30:]
        + reference_vntr.pattern
        + reference_vntr.right_flanking_region[:30]
    )
    ...
    command = (
        "blat -q=dna -oneOff=1 -tileSize=8 -stepSize=3 -minIdentity=75 %s %s %s"
...
def identify_similar_regions_for_vntrs_using_blat():
    ...
    for ref_vntr in reference_vntrs:
        sema.acquire()
        p = Process(
            target=find_similar_region_for_vntr,
...
def extend_flanking_regions_in_processed_vntrs(
    flanking_size=500, output_file="vntr_data/repeats_and_segments2.txt"
):
    ...
    for vntr in vntrs:
        ...
        if vntr.chromosome not in reference_genomes.keys():
            reference_genomes[vntr.chromosome] = get_chromosome_reference_sequence(
                vntr.chromosome
            )
        start = vntr.start_point
        left_flanking_region = reference_genomes[vntr.chromosome][
            start - flanking_size : start
        ].upper()
        end = vntr.start_point + vntr.get_length()
        right_flanking_region = reference_genomes[vntr.chromosome][
...
if __name__ == "__main__":
    ...
    for i, chrom in enumerate(settings.CHROMOSOMES):
        if i != int(sys.argv[1]):
            continue
        sorted_vntrseek_output = (
            PROCESSING_DIR + "/repeats_length_patterns_chromosomes_starts.txt"
        )
        processed_vntrs = PROCESSING_DIR + "/VNTRs_%s.txt" % chrom
