...
class PacBioHaplotyper:
    ...
    def get_error_corrected_haplotypes(self, number_of_clusters=2):
        if len(self.reads) < 2:
            logging.info(
                "There is only one spanning read. Returning it as the only available haplotype"
            )
            return self.reads
        haplotypes = []
        ...
        for cluster in clusters:
            ...
            if len(cluster) < 2:
                logging.debug(
                    "Cluster has only one sequence. Skipping multiple alignment"
                )
    ...
    def get_read_clusters(self, number_of_clusters=2):
        ...
        result = [
            [self.reads[int(aligned_read_ids[i])] for i in cluster]
            for cluster in clusters
        ]
        return result
