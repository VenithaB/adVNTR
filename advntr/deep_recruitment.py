...
OUTPUT_DIR_PREFIX = (
    "../advntr2_recruitment_comparison_hg38/"
    if hg38
    else "../advntr2_recruitment_comparison/"
)
...
result_dir = OUTPUT_DIR_PREFIX + "hmm_dnn_comparison_%s/" % (
    str(kmer_length) + loss_suffix
)
...
def get_blast_keywords(reference_vntr, keyword_size=11):
    ...
    locus = (
        reference_vntr.left_flanking_region[-15:]
        + vntr
        + reference_vntr.right_flanking_region[:15]
    )
    queries = []
...
def get_hmm_accuracy(vntr_finder, simulated_true_reads, simulated_false_filtered_reads):
    ...
    search_id = "search_id"
    search_results = get_blast_matched_ids(
        query,
    ...
    output_dir = result_dir + "/%s/" % vntr_finder.reference_vntr.id
    if (
        os.path.exists(output_dir + "/hmm.txt")
        and os.path.getsize(output_dir + "/hmm.txt") > 0
    ):
    ...
    train_false_reads = [
        read for i, read in enumerate(simulated_false_filtered_reads) if i % 2 == 0
    ]
    test_true_reads = [
        read for i, read in enumerate(simulated_true_reads) if i % 2 == 1
    ]
    test_false_reads = [
        read for i, read in enumerate(simulated_false_filtered_reads) if i % 2 == 1
    ]
...
def simulate_and_store_false_reads(vntr_finder, false_reads_file, min_matches=4):
    ...
    for reference_file in reference_files:
        simulated_false_filtered_reads += vntr_finder.simulate_false_filtered_reads(
            reference_file, min_matches
        )
        print(len(simulated_false_filtered_reads))
...
def get_true_reads_and_false_reads(vntr_finder, vntr_id):
    ...
    while True:
        ...
        if len(lines) > 40000:
            print(
                "There are more than %s reads in the file. Trying min_matches = %s"
                % (len(lines), min_matches)
            )
            simulate_and_store_false_reads(vntr_finder, false_reads_file, min_matches)
            min_matches += 2
        else:
            break
    simulated_false_filtered_reads = [
        read.strip() for read in lines if "N" not in read.upper()
    ]
...
def get_nn_model(
    ...
    if second_layer > 0:
        model.add(
            Dense(second_layer, activation=model_function, kernel_initializer="uniform")
        )
    model.add(Dense(2))
...
def select_positive_and_negative_reads_with_bowtie(reads, vntr_finder, label):
    ...
    bowtie_selected = len(
        get_id_of_reads_mapped_to_vntr_in_bamfile(
            bowtie_bamfile, vntr_finder.reference_vntr
        )
    )
    return (
        float(bowtie_selected),
        float(len(reads) - bowtie_selected),
        time() - passed_time,
    )
...
def run_bowtie2(true_reads, false_reads, vntr_finder):
    ...
    if (
        os.path.exists(output_dir + "/bowtie.txt")
        and os.path.getsize(output_dir + "/bowtie.txt") > 0
    ):
    ...
    train_time = 0
    tp, fn, t1 = select_positive_and_negative_reads_with_bowtie(
        true_reads, vntr_finder, "true"
    )
    fp, tn, t2 = select_positive_and_negative_reads_with_bowtie(
        false_reads, vntr_finder, "false"
    )
    passed_time = t1 + t2
    precision = tp / (tp + fp) if tp > 0 else 0
    recall = tp / (tp + fn)
...
def run_simulation(vntr_map, vntr_id):
    ...
    simulated_true_reads, simulated_false_filtered_reads = (
        get_true_reads_and_false_reads(vntr_finder, vntr_id)
    )
    if (
        len(simulated_false_filtered_reads) > 30000
        or len(simulated_false_filtered_reads) == 0
    ):
    ...
    if (
        os.path.exists(output_dir + "dnn.txt")
        and os.path.getsize(output_dir + "dnn.txt") > 0
    ):
    ...
    false_embeddings = [
        get_embedding_of_string(seq) for seq in simulated_false_filtered_reads
    ]
    ...
    for i in range(len(test[1])):
        majority = int(
            is_true(classes[i])
        )  # + int(is_true(classes2[i])) + int(is_true(classes3[i]))
        # print(majority)
        if test[1][i][0] == 1:
            if majority >= 1:
                tp += 1
