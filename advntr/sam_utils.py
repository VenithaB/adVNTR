...
@time_usage
def extract_unmapped_reads_to_fasta_file(
    alignment_file,
    working_directory="./",
    ref_filename=None,
    ...
    if not use_existing_computed_files or not os.path.exists(unmapped_read_file):
        if not use_existing_computed_files or not os.path.exists(unmapped_bam_file):
            if ref_filename is not None and alignment_file.endswith("cram"):
                os.system(
                    "samtools view -T %s -b -f 4 %s > %s"
                    % (ref_filename, alignment_file, unmapped_bam_file)
                )
            else:
                os.system(
                    "samtools view -b -f4 %s > %s" % (alignment_file, unmapped_bam_file)
                )
        os.system(
            "samtools bam2fq %s | %s > %s"
            % (unmapped_bam_file, fastq_to_fasta_command, unmapped_read_file)
        )
...
def make_bam_and_index(samfile):
    bamfile = samfile[:-4]
    os.system(
        "samtools view -bS %s | samtools sort - > %s" % (samfile, bamfile + ".bam")
    )
...
def get_reads_mapped_to_vntr_in_samfile(
    sam_file, reference_vntr, read_length=150, region=None
):
...
def get_id_of_reads_mapped_to_vntr_in_samfile(
    sam_file, reference_vntr, read_length=150, region=None
):
    reads = get_reads_mapped_to_vntr_in_samfile(
        sam_file, reference_vntr, read_length, region=region
    )
    return [read.qname for read in reads]


def get_related_reads_and_read_count_in_samfile(
    pattern, pattern_start, repeats=None, read_file="", pattern_end=None
):
...
def get_vntr_coverage_over_total_coverage(
    start_point, end_point, read_file="original_reads/paired_dat.sam"
):
    samfile = pysam.AlignmentFile(read_file, "r")
    read_count = 0
    avg_read_size = 0
    ...
    for read in samfile.fetch():
        ...
        read_count += 1
        if (
            start_point <= read.reference_start < end_point
            or start_point < read.reference_end <= end_point
        ):
