...
def get_gene_name_and_annotation_of_vntr(
    vntr_chromosome,
    vntr_start,
    vntr_end,
    genes,
    ...
    if gene_name == "None":
        gene_name, annotation = get_annotation(
            vntr_start, vntr_end, utr5[vntr_chromosome], "UTR"
        )
    if gene_name == "None":
        gene_name, annotation = get_annotation(
            vntr_start, vntr_end, utr3[vntr_chromosome], "UTR"
        )
    if gene_name == "None":
        gene_name, annotation = get_annotation(
            vntr_start, vntr_end, introns[vntr_chromosome], "Intron"
        )
...
def get_exons_info(annotation_file=EXONS, gene_reference="refseq"):
    ...
    with open(annotation_file % gene_reference) as infile:
        ...
        for line in lines:
            ...
            exons_info[chromosome].append(
                (start, end, identifier, direction, segment_number)
            )
...
def get_RepeatMasker_info(repeat_ref_file):
    ...
    with open(repeat_ref_file, "r") as infile:
        for line in infile:
            ...
            repeat_info[chromosome].append(
                (int(genoStart), int(genoEnd), strand, repName, repClass, repFamily)
            )
...
def is_within_LINE_or_SINE(vntr_chromosome, vntr_start, vntr_end, repeat_masker_info):
    for (
        ...
        repClass,
        repFamily,
    ) in repeat_masker_info[vntr_chromosome]:
...
if __name__ == "__main__":
    from models import (
        update_gene_name_and_annotation_in_database,
    ...
    for ref_vntr in reference_vntrs:
        ...
        print(ref_vntr.id)
        update_gene_name_and_annotation_in_database(
            ref_vntr.id, new_gene, new_annotation, db_file
        )
