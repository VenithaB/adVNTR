...
def build_profile_hmm_pseudocounts_for_alignment(error_rate, alignment):
    ...
    if "I" + str(len(alignment[0]) - len(insert_index)) not in transition:
        ...
        if (
            "unit_end"
            not in transition["I" + str(len(alignment[0]) - len(insert_index))]
        ):
    ...
    if "D" + str(len(alignment[0]) - len(insert_index)) not in transition:
        ...
        if (
            "unit_end"
            not in transition["D" + str(len(alignment[0]) - len(insert_index))]
        ):
    ...
    if "M" + str(len(alignment[0]) - len(insert_index)) not in transition:
        ...
        if (
            "unit_end"
            not in transition["M" + str(len(alignment[0]) - len(insert_index))]
        ):
        ...
        if key != "unit_end":
            ...
            for sub_key in transition[key]:
                if total > 0:
                    ...
                    transition[key][sub_key] = (transition[key][sub_key] + pseu) / (
                        1 + pseu * len(transition[key])
                    )
                else:
