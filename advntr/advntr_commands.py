...
from advntr.models import (
    load_unique_vntrs_data,
    get_largest_id_in_database,
    save_reference_vntr_to_database,
)
...
def get_default_vntrs(reference_vntrs, is_pacbio=False):
    ...
    for ref_vntr in reference_vntrs:
        if not ref_vntr.is_non_overlapping() or ref_vntr.has_homologous_vntr():
            continue
        if (
            "N" in ref_vntr.left_flanking_region[-100:]
            or "N" in ref_vntr.right_flanking_region[:100]
        ):
            continue

        if ref_vntr.get_length() < 140 and ref_vntr.annotation in [
            "Coding",
            "UTR",
            "Promoter",
        ]:
        ...
        pacbio = (
            True
            if illumina or ref_vntr.annotation in ["Coding", "UTR", "Promoter"]
            else False
        )
...
def genotype(args, genotype_parser):
    ...
    input_is_alignment_file = (
        input_file.endswith("bam")
    ...
    logging.basicConfig(
        format=log_format, filename=log_file, level=logging.DEBUG, filemode="w"
    )
    ...
    settings.TRAINED_HMMS_DIR = (
        os.path.dirname(os.path.realpath(settings.TRAINED_MODELS_DB)) + "/"
    )
    ...
    logging.info("Reference VNTR DB: {}".format(settings.TRAINED_MODELS_DB))
    logging.info("Running adVNTR for %s VNTRs" % len(target_vntrs))
    genome_analyzier = GenomeAnalyzer(
        reference_vntrs,
        target_vntrs,
        working_directory,
        args.outfmt,
    ...
    if args.pacbio:
        if input_is_alignment_file:
            genome_analyzier.find_repeat_counts_from_pacbio_alignment_file(
                input_file, args.log_pacbio_reads, args.accuracy_filter
            )
        else:
            genome_analyzier.find_repeat_counts_from_pacbio_reads(
                input_file, args.log_pacbio_reads, args.accuracy_filter, args.naive
            )
    else:
        if args.frameshift:
            if valid_vntr_for_frameshift(target_vntrs):
                ...
                print_error(
                    genotype_parser, "--frameshift is not available for these VNTRs"
                )
            ...
            genome_analyzier.find_repeat_counts_from_short_reads(
                read_file=input_file, accuracy_filter=args.accuracy_filter
            )
