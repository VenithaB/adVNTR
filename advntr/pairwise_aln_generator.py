    ...
    for repeat in sorted(repeat_seq_dict.keys()):
        ...
        for idx, (
            ...
            if flanking_repeats_used_in_genotyping is not None:
                ...
                if (
                    not is_spanning_read
                    and repeat != flanking_repeats_used_in_genotyping
                ):
                    continue
            ...
            for state in visited_states:
                ...
                if "end" in state:
                    if "unit_end" in state:
                        ...
                        if observed_first_unit_start:
                            if (
                                repeat_unit_seq != ""
                            ):  # Check for erroneous case having all deletion states
                                best_aln = find_best_repeat_unit(
                                    repeat_unit_seq, unique_patterns
                                )
                                query_seq = query_seq[
                                    : len(query_seq) - repeat_unit_state_count
                                ]
                                query_seq += best_aln[0]

                                match_line = match_line[
                                    : len(match_line) - repeat_unit_state_count
                                ]
                                match_line += get_match_line(best_aln)

                                ref_seq = ref_seq[
                                    : len(ref_seq) - repeat_unit_state_count
                                ]
                                ref_seq += best_aln[1]
                ...
                if "suffix" in state:
                    ...
                    if state_name == "M":
                        ...
                        if (
                            sequence[seq_index]
                            == left_flank[-(max_hmm_index - hmm_index + 1)]
                        ):
                            match_line += "|"
                        else:
                            match_line += " "
                    ...
                    if state_name == "M":
                        if observed_first_unit_start:
                            repeat_unit_seq += sequence[
                                seq_index
                            ]  # Do it only when the first unit is observed
            ...
            af.write(
                ...
                " L:{}/{} {:.2f}, R:{}/{} {:.2f}\n".format(
                    ...
                    (
                        ...
                        / (left_flank_bp_count + right_flank_bp_count)
                        if left_flank_bp_count != 0 or right_flank_bp_count != 0
                        else 0
                    ),
                    mismatch_count_in_left_flanking,
                    left_flank_bp_count,
...
def _generate_pairwise_aln(
    log_file,
    aln_outfile,
    ref_vntrs,
    vid_list=None,
    sort_by_repeat=True,
    print_only_informative_flanking_read=True,
):
    ...
    with open(log_file, "r") as f:
        for line in f:
            ...
            if (
                "DEBUG:finding" in line
            ):  # DEBUG:finding repeat count from alignment file for [vid]
            ...
            if is_target:
                if is_frameshift_result:
                    ...
                    if (
                        "DEBUG:VisitedStates:" in line
                    ):  # DEBUG:VisitedStates:['state1', ...]
                    ...
                    if "DEBUG:repeats" in line:  # DEBUG:repeats: [repeat_count]
                        ...
                        vid_to_aln_info[vid][repeats].append(
                            (
                                sequence,
                                visited_states,
                                is_spanning_read,
                        ...
                        if (
                            read_id == "visited"
                        ):  # read id and source is not available in this log file
                            read_id = ""
                            read_source = ""
                        else:
            ...
            if "RU count lower bounds" in line:
                if print_only_informative_flanking_read:
                    min_valid_flanked = (
                        max(spanning_repeats) if len(spanning_repeats) > 0 else 0
                    )
                    max_flanking_repeat = [
                        r
                    ...
                    vid_flanking_repeats_used_in_genotyping[vid] = (
                        max(flanking_repeats) if len(max_flanking_repeat) >= 5 else None
                    )

    if sort_by_repeat:
        vid_to_aln_info = {
            k: v for k, v in sorted(vid_to_aln_info.items(), key=lambda k_v: k_v[1][1])
        }
    ...
    with open(aln_outfile, "w") as af:
        for vid in sorted(vid_to_aln_info.keys()):
            if ref_vntrs.get(vid) is None:
                ...
                af.write(
                    "ERROR: The reference VNTR is not in the DB, VID: {}\n".format(vid)
                )
                continue
            write_alignment(
                af,
                ...
                ref_vntrs[vid],
                vid_read_length[vid],
                is_frameshift_result,
                vid_flanking_repeats_used_in_genotyping.get(vid),
            )


def generate_pairwise_aln(
    log_file, aln_file, ref_vntr_db=None, vntr_ids=None, sort_by_repeat=True
):
    """
...
def _update_count_dictionary(
    ref_vntr,
    repeats,
    visited_states,
    sequence,
    repeat_flanking_errcount,
    repeat_flanking_bpcount,
):
...
def _get_flanking_region_error_rate(log_file, ref_vntrs, vid_list):
    vid_repeat_flanking_errcount = defaultdict(
        lambda: defaultdict(lambda: defaultdict(int))
    )
    vid_repeat_flanking_bpcount = defaultdict(
        lambda: defaultdict(lambda: defaultdict(int))
    )
    is_target = False if vid_list is not None else True
    visited_states = None
    ...
    with open(log_file, "r") as f:
        for line in f:
            ...
            if is_target:
                if "DEBUG:repeats" in line:  # DEBUG:repeats: [repeat_count]
                    repeats = int(line.strip().split(" ")[-1])
                    _update_count_dictionary(
                        ref_vntrs[vid],
                        repeats,
                        visited_states,
                        sequence,
                        vid_repeat_flanking_errcount[vid],
                        vid_repeat_flanking_bpcount[vid],
                    )
                ...
                for repeat_count in bpcount_dict[vid].keys():
                    for flanking in bpcount_dict[vid][repeat_count].keys():
                        total_vid_repeat_flanking_errcount[vid][repeat_count][
                            flanking
                        ] += errcount_dict[vid][repeat_count][flanking]
                        total_vid_repeat_flanking_bpcount[vid][repeat_count][
                            flanking
                        ] += bpcount_dict[vid][repeat_count][flanking]
    else:
        total_vid_repeat_flanking_errcount, total_vid_repeat_flanking_bpcount = (
            _get_flanking_region_error_rate(log_file, ref_vntrs, vntr_ids)
        )
...
def main():
    ...
    if arg_dict["stat"]:
        if arg_dict["o"] is None:
            ...
            exit(-1)
        get_flanking_region_error_rate(
            arg_dict["i"], arg_dict["o"], arg_dict["db"], vntr_ids=target_vntrs_ids
        )
    else:
        generate_pairwise_aln(
            arg_dict["i"],
            arg_dict["o"],
            arg_dict["db"],
            vntr_ids=target_vntrs_ids,
            sort_by_repeat=True,
        )
